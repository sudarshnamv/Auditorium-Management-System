...
Public Class Updateuser
    ...
    Private Sub txtSearch_TextChanged(sender As Object, e As EventArgs) Handles txtSearch.TextChanged
        'Search for users whose empid matches the search string
        adapter = New SqlDataAdapter("SELECT * FROM userregistration WHERE empid LIKE '%" & txtSearch.Text & "%'", con)
        table.Clear()
        adapter.Fill(table)
        DataGridView1.DataSource = table
    ...
    Private Sub btnSave_Click(sender As Object, e As EventArgs) Handles btnSave.Click
        ...
        Else
            Dim cmd As New SqlCommand("UPDATE userregistration SET password=@password, email=@email, phone=@phone WHERE empid=@empid", con)
            cmd.Parameters.AddWithValue("@empid", DataGridView1.CurrentRow.Cells("empid").Value.ToString())
